...
static int colorization(const cv::Mat& bgr)
{
    ...
    cv::imwrite("images_output/colored_out.png", color);
//  imshow("color", color);
//  cv::waitKey();
int preview_width = 800;
    if (color.cols > preview_width) {
        float scale = preview_width / (float)color.cols;
        cv::Mat preview;
        cv::resize(color, preview, cv::Size(), scale, scale);
        cv::imwrite("images_output/preview_out.png", preview);
    } else {
        cv::imwrite("images_output/preview_out.png", color);
    }
